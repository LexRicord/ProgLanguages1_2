...
FST::NODE::NODE(short n, RELATION rel, ...)
{
	...
	RELATION* ptr = &rel;
	for (short i = 0; i < n; i++) this->relations[i] = *ptr++;
}
...
bool FST::step(FST& fst, short* rstates)   //step on transition graph
{
	swap(fst.rstates, rstates );  //swap obj. between one another
	
	bool real = false;			//possibility of step further in graph
